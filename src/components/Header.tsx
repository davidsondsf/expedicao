...
export function Header({ title }: HeaderProps) {
  ...
  const handleSearch = async (val: string) => {
    ...
    if (val.trim().length > 1) {
      const { data } = await supabase
        ...
        .or(`name.ilike.%${val}%,barcode.ilike.%${val}%`)
        .eq('active', true)
        .limit(5);
      setResults(data ?? []);
    } else {
  ...
  const handleLogout = async () => {
    await log({ action: 'LOGOUT', entity: 'auth', entityId: user?.id });
    await logout();
    navigate('/login');
  };
            ...
            {results.map(item => (
              ...
              >
                ...
                <span className={cn(
                  ...
                  item.quantity <= item.min_quantity ? 'text-destructive' : 'text-[hsl(var(--success))]'
                )}>
                  {item.quantity} un
                </span>
              </button>
