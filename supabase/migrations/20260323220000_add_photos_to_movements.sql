-- Migration: Add photos array to movements table

ALTER TABLE public.movements
ADD COLUMN IF NOT EXISTS photos text[] DEFAULT '{}'::text[];

DROP FUNCTION IF EXISTS public.create_movement_and_adjust_stock(uuid, text, integer, uuid, text);
...
BEGIN
  ...
  -- Insert movement
  INSERT INTO public.movements (item_id, user_id, type, quantity, note, photos)
  VALUES (p_item_id, p_user_id, p_type, p_quantity, p_note, COALESCE(p_photos, '{}'::text[]));
